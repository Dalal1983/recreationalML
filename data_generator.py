...
input_characters = " ".join(list(target_to_text.values()))
...
def get_batch(batch_size=100, min_digits = 3, max_digits=3):
    ...
    for i in range(batch_size):
        ...
        int_target_in = [valid_characters.index(c) for c in text_target_in]
        int_target_out = [valid_characters.index(c) for c in text_target_out]

        #generate the text input
        text_input = " ".join([target_to_text[k] for k in inp_str])
        #generate the inputs as a list of intergers
        int_input = [valid_characters.index(c) for c in text_input]
